  ...
  return (
    <article className="group bg-secondary-100 rounded-lg shadow-lg p-4 transition transform hover:scale-105">
      <div className="relative bg-secondary-200">
        ...
        <Link
          to={`/songs/${song._id}`}
          className="absolute inset-0 flex items-center justify-center rounded-lg bg-primary bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out"
        ></Link>
        <button
          className={`bg-${selectedTheme} absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2 top-1/2 bg-opacity-20 text-white flex justify-center items-center w-14 h-14 active:bg-${selectedTheme}-50 active:bg-opacity-40 rounded-full transition`}
        >
